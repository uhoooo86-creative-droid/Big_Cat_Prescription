...
// 아래 두 줄을 실제 값으로 교체하세요
// ================================================

import { useState, useEffect } from "react";

const SUPABASE_URL = "https://lcnowfefkwtexkxtbazp.supabase.co";   // ← 교체
...
async function submitInquiry({ carId, dealerId, buyerName, buyerPhone, message }) {
  return sb.insert("inquiries", {
    car_id: carId, dealer_id: dealerId,
...
async function registerCar({ dealerId, form, options }) {
  const [car] = await sb.insert("cars", {
    dealer_id: dealerId,
    brand: "Mercedes-Benz",
    model: form.model, trim: form.trim,
    ...
    accident_free: form.accidentFree, reg_by: form.regBy,
    status: "pending",
  });
  if (options.length > 0) {
    await sb.insert("car_options", options.map(name => ({ car_id: car.id, option_name: name })));
  }
  return car;
}
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    try {
      const [d, approved, pending] = await Promise.all([
        fetchDealer(), fetchCars("approved"), fetchCars("pending"),
      ]);
      setDealer(d); setApprovedCars(approved); setPendingCars(pending);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const handleInquiry = async () => {
    ...
    try {
      await submitInquiry({ carId: selectedCar.id, dealerId: dealer.id, buyerName: inquiryForm.name, buyerPhone: inquiryForm.phone, message: inquiryForm.message });
      setInquiryDone(true);
  ...
  const handleApprove = async (carId) => {
    try { await approveCar(carId); await loadData(); }
    ...
    <div style={{
      ...
      <div style={{ paddingBottom: 80 }}>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "80px 20px" }}>
            <div style={{ width: 32, height: 32, border: "3px solid #1C1C1C", borderTopColor: "#C9A84C", borderRadius: "50%", animation: "spin 1s linear infinite", margin: "0 auto 16px" }} />
            <div style={{ fontSize: 13, color: "#555" }}>데이터 로딩중...</div>
          </div>
        )}
        ...
        {!loading && error && (
          <div style={{ margin: 20, background: "#1A0808", border: "1px solid #F03E3E44", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ fontSize: 11, color: "#AA8888", lineHeight: 1.7, marginBottom: 16 }}>
              src/App.js 상단의 SUPABASE_URL과 SUPABASE_KEY를 실제 값으로 교체해주세요.
            </div>
            <button onClick={loadData} style={{ background: "#C9A84C", color: "#0A0A0A", fontSize: 12, padding: "8px 16px", borderRadius: 8, fontWeight: 700, fontFamily: "'Noto Sans KR'" }}>다시 시도</button>
          </div>
        )}
        ...
        {!loading && !error && (
          <>
              ...
              <div className="fadeUp">
                <div style={{ padding: "32px 24px 24px", borderBottom: "1px solid #1C1C1C" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, background: "#111", border: "1px solid #1C1C1C", borderRadius: 10, padding: "10px 14px", alignItems: "center" }}>
                    ...
                    />
                    <button onClick={() => setView("list")} style={{ background: "#C9A84C", color: "#0A0A0A", fontSize: 12, padding: "5px 14px", borderRadius: 6, fontFamily: "'Noto Sans KR'", fontWeight: 700 }}>검색</button>
                  </div>
                </div>
                ...
                <div style={{ padding: "20px 20px 8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>
                      등록 매물 <span style={{ fontSize: 11, color: "#C9A84C", fontFamily: "'Space Mono'" }}>{approvedCars.length}</span>
                    </div>
              ...
              <div className="fadeUp">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, padding: "16px 20px 0" }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: "#111", border: "1px solid #1C1C1C", borderRadius: 12, padding: "14px 12px", textAlign: "center" }}>
                ...
                {pendingCars.length > 0 && (
                  <div style={{ padding: "20px 20px 0" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#F03E3E", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#F03E3E", animation: "pulse 1.5s infinite" }} />
                      검수 대기 차량
                    </div>
                    {pendingCars.map(car => (
                      <div key={car.id} style={{ background: "#111", border: "1px solid #F03E3E22", borderRadius: 12, padding: 14, marginBottom: 10 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                          <div>
                            ...
                            <div style={{ fontSize: 11, color: "#666" }}>
                              ...
                              <span style={{ color: "#C9A84C", marginLeft: 4 }}>{car.reg_by === "owner" ? "차주 등록" : "딜러 등록"}</span>
                            </div>
                          </div>
                          <div style={{ fontSize: 16, fontWeight: 900, color: "#C9A84C", fontFamily: "'Space Mono'" }}>{car.price?.toLocaleString()}만</div>
                        </div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={() => handleApprove(car.id)} className="tap" style={{ flex: 1, padding: "9px 0", borderRadius: 8, background: "#C9A84C", color: "#0A0A0A", fontSize: 13, fontWeight: 900, fontFamily: "'Noto Sans KR'" }}>✓ 승인</button>
                          <button style={{ flex: 1, padding: "9px 0", borderRadius: 8, background: "#1A1A1A", border: "1px solid #222", color: "#888", fontSize: 13, fontFamily: "'Noto Sans KR'" }}>✕ 반려</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ padding: "20px 20px 0" }}>
                  <button onClick={() => { setRegStep(1); setSelectedOptions([]); setRegForm({ brand: "Mercedes-Benz", model: "", trim: "", year: "", mileage: "", fuel: "가솔린", transmission: "자동", price: "", description: "", accidentFree: true, regBy: "dealer" }); setView("register"); }} className="tap" style={{ width: "100%", padding: "16px 0", borderRadius: 12, background: "linear-gradient(135deg, #C9A84C, #A07830)", color: "#0A0A0A", fontSize: 15, fontWeight: 900, fontFamily: "'Noto Sans KR'" }}>
                    + 새 차량 등록하기
                  </button>
                </div>
              </div>
            ...
            {view === "detail" && selectedCar && (
              <div>
                <button onClick={() => setView(mode === "dealer" ? "home" : "list")} style={{ padding: "14px 20px", fontSize: 13, color: "#666", display: "flex", alignItems: "center", gap: 5, fontFamily: "'Noto Sans KR'" }}>← 돌아가기</button>

                <div style={{ margin: "0 20px", height: 180, borderRadius: 14, background: "linear-gradient(135deg, #C9A84C18, #0A0A0A)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 72, border: "1px solid #C9A84C22", position: "relative" }}>
                  🏎️
                ...
                <div style={{ padding: "20px 20px 0" }}>
                  ...
                  {mode === "buyer" && (
                    <div style={{ display: "flex", gap: 10, paddingBottom: 24, marginTop: 16 }}>
                      <button onClick={() => toggleFav(selectedCar.id)} style={{ flex: 1, padding: "13px 0", borderRadius: 10, background: "#111", border: "1px solid #222", color: favorites.includes(selectedCar.id) ? "#C9A84C" : "#555", fontSize: 14, fontFamily: "'Noto Sans KR'", fontWeight: 700 }}>
                        {favorites.includes(selectedCar.id) ? "❤️ 찜됨" : "🤍 찜"}
                      </button>
                      <button onClick={() => { setInquiryDone(false); setInquiryForm({ name: "", phone: "", message: "" }); setView("inquiry"); }} className="tap" style={{ flex: 2, padding: "13px 0", borderRadius: 10, background: "linear-gradient(135deg, #C9A84C, #A07830)", color: "#0A0A0A", fontSize: 14, fontWeight: 900, fontFamily: "'Noto Sans KR'" }}>
                        딜러에게 문의하기
                      </button>
                    </div>
                  )}
            ...
            {view === "inquiry" && (
              <div>
                <button onClick={() => setView("detail")} style={{ padding: "14px 20px", fontSize: 13, color: "#666", display: "flex", alignItems: "center", gap: 5, fontFamily: "'Noto Sans KR'" }}>← 차량 정보</button>
                  ...
                  {!inquiryDone ? (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: "#555", marginBottom: 20 }}>
                        <span style={{ color: "#C9A84C" }}>{dealer?.name} {dealer?.title}</span>이 직접 연락드립니다.
                      </div>
                      ...
                      <button onClick={handleInquiry} disabled={inquiryLoading} className="tap" style={{ width: "100%", padding: "15px 0", borderRadius: 12, background: inquiryLoading ? "#888" : "linear-gradient(135deg, #C9A84C, #A07830)", color: "#0A0A0A", fontSize: 15, fontWeight: 900, fontFamily: "'Noto Sans KR'", marginBottom: 24 }}>
                        {inquiryLoading ? "전송 중..." : "상담 신청하기"}
                      </button>
                    </>
                  ) : (
            ...
            {view === "register" && (
              <div>
                <button onClick={() => setView("home")} style={{ padding: "14px 20px", fontSize: 13, color: "#666", display: "flex", alignItems: "center", gap: 5, fontFamily: "'Noto Sans KR'" }}>← 취소</button>
                <div style={{ padding: "0 20px" }}>
                    ...
                    <div className="fadeUp">
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
                        {[{ val: "dealer", label: "딜러 직접 등록" }, { val: "owner", label: "차주 위탁 등록" }].map(opt => (
                          <button key={opt.val} onClick={() => setRegForm(f => ({ ...f, regBy: opt.val }))} style={{ flex: 1, padding: "13px 10px", borderRadius: 10, background: regForm.regBy === opt.val ? "#C9A84C11" : "#111", border: "1px solid", borderColor: regForm.regBy === opt.val ? "#C9A84C" : "#1C1C1C", color: regForm.regBy === opt.val ? "#C9A84C" : "#666", fontFamily: "'Noto Sans KR'", fontWeight: 700, fontSize: 13 }}>{opt.label}</button>
                        ))}
                      </div>
                      {[
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        {[
                          ...
                          <div key={f.key}>
                            <div style={{ fontSize: 11, color: "#555", marginBottom: 5 }}>{f.label}</div>
                            <select value={regForm[f.key]} onChange={e => setRegForm(p => ({ ...p, [f.key]: e.target.value }))} style={{ width: "100%", background: "#111", border: "1px solid #1C1C1C", borderRadius: 10, padding: "11px 13px", color: "#F0EDE8", fontSize: 13, fontFamily: "'Noto Sans KR'" }}>
                  ...
                  {regStep === 2 && (
                    <div className="fadeUp">
                      ...
                      {["파노라마 선루프", "버메스터 사운드", "어댑티브 크루즈", "어라운드뷰", "HUD", "마사지 시트", "나이트비전", "열선시트", "통풍시트", "후방카메라"].map(opt => (
                        <label key={opt} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 13px", background: selectedOptions.includes(opt) ? "#C9A84C0A" : "#111", borderRadius: 8, marginBottom: 6, cursor: "pointer", border: `1px solid ${selectedOptions.includes(opt) ? "#C9A84C33" : "#1C1C1C"}` }}>
                          <span style={{ fontSize: 13, color: selectedOptions.includes(opt) ? "#C9A84C" : "#CCC" }}>{opt}</span>
                          <input type="checkbox" checked={selectedOptions.includes(opt)} onChange={() => toggleOption(opt)} style={{ width: 16, height: 16, accentColor: "#C9A84C" }} />
                        </label>
                      ))}
                    </div>
                  )}
                  ...
                  {regStep === 3 && (
                    <div className="fadeUp">
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        {["무사고", "사고 있음"].map(opt => (
                          <button key={opt} onClick={() => setRegForm(f => ({ ...f, accidentFree: opt === "무사고" }))} style={{ flex: 1, padding: "12px 0", borderRadius: 10, background: (opt === "무사고") === regForm.accidentFree ? "#C9A84C" : "#111", border: "1px solid", borderColor: (opt === "무사고") === regForm.accidentFree ? "#C9A84C" : "#1C1C1C", color: (opt === "무사고") === regForm.accidentFree ? "#0A0A0A" : "#666", fontFamily: "'Noto Sans KR'", fontWeight: 700, fontSize: 13 }}>{opt}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {regStep === 5 && (
                    <div className="fadeUp" style={{ textAlign: "center", padding: "20px 0" }}>
                      ...
                      <div style={{ fontSize: 13, color: "#555", lineHeight: 1.7, marginBottom: 28 }}>검수 대기 상태로 저장됐어요.<br />대시보드에서 승인하면 즉시 공개됩니다.</div>
                      <button onClick={() => { setView("home"); setRegStep(1); }} className="tap" style={{ width: "100%", padding: "14px 0", borderRadius: 12, background: "linear-gradient(135deg, #C9A84C, #A07830)", color: "#0A0A0A", fontSize: 14, fontWeight: 900, fontFamily: "'Noto Sans KR'" }}>대시보드에서 확인하기</button>
                    </div>
                  )}
                  ...
                  {regStep < 5 && (
                    <div style={{ display: "flex", gap: 10, marginTop: 24, paddingBottom: 24 }}>
                      {regStep > 1 && (
                        <button onClick={() => setRegStep(s => s - 1)} style={{ flex: 1, padding: "13px 0", borderRadius: 12, background: "#111", border: "1px solid #1C1C1C", color: "#888", fontSize: 13, fontFamily: "'Noto Sans KR'" }}>이전</button>
                      )}
                      <button onClick={regStep === 4 ? handleRegister : () => setRegStep(s => s + 1)} disabled={regLoading} className="tap" style={{ flex: 2, padding: "13px 0", borderRadius: 12, background: regLoading ? "#888" : "linear-gradient(135deg, #C9A84C, #A07830)", color: "#0A0A0A", fontSize: 14, fontWeight: 900, fontFamily: "'Noto Sans KR'" }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "rgba(10,10,10,0.97)", backdropFilter: "blur(16px)", borderTop: "1px solid #1A1A1A", display: "flex", justifyContent: "space-around", padding: "10px 0 16px", zIndex: 100 }}>
        {(mode === "buyer" ? [
          ...
          <button key={tab.id} onClick={() => setView(tab.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "4px 20px" }}>
...
function CarRow({ car, fav, onFav, onClick, delay = 0, dealerMode = false }) {
  return (
    <div className="tap fadeUp" onClick={onClick} style={{ background: "#111", border: "1px solid #1C1C1C", borderRadius: 14, overflow: "hidden", cursor: "pointer", marginBottom: 12, animationDelay: `${delay}s` }}>
      <div style={{ height: 120, background: "linear-gradient(135deg, #C9A84C18, #0A0A0A)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56, position: "relative", borderBottom: "1px solid #161616" }}>
        🏎️
...
function Badge({ text, color }) {
  return (
    <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${color}22`, color, border: `1px solid ${color}44` }}>{text}</span>
  );
}
